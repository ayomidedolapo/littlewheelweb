...
export async function POST(req: NextRequest) {
	try {
		...
		if (response.data.message?.includes('already') || response.data.message?.includes('existing')) {
		...
		return NextResponse.json({ error: response.data.message || 'Failed to add' }, { status: 500 });
	} catch (error) {
		console.error('Error in POST /api/join-waitlist:', error);

		// Check if the error response contains our duplicate message
		if (error instanceof AxiosError && error.response?.data?.message?.includes('already')) {
			return NextResponse.json({ error: 'This email is already subscribed' }, { status: 409 });
		}
